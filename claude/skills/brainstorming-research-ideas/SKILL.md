...
**alphaxiv MCP Tool Integration**:
When executing this skill, use the following alphaxiv MCP tools to ground ideation in real data:

1. **Trend Discovery**: Use `mcp__alphaxiv__find_papers_feed` to identify hot topics and emerging directions
   - Filter by relevant topics (cs.RO, cs.CV, cs.LG, etc.)
   - Compare Hot vs Views rankings to distinguish hype from substance
   - Use different intervals (7 Days vs 90 Days) to spot accelerating trends

2. **Validate Ideas**: Use `mcp__alphaxiv__answer_research_query` to check novelty and feasibility
   - Query "Has anyone done X?" before committing to an idea
   - Query "What are the limitations of Y?" to find genuine gaps
   - Query "Recent approaches to Z" to understand the competitive landscape

3. **Deep Dive**: Use `mcp__alphaxiv__answer_pdf_queries` to analyze specific papers that relate to candidate ideas
   - Extract failure modes and limitations as inspiration for new ideas
   - Identify unexplored combinations of existing techniques

4. **Code Feasibility**: Use `mcp__alphaxiv__read_files_from_github_repository` to assess implementation complexity of related work
